...
def sweepScreen(m):
    im = captureMss(m)
    res = locate("images/captcha/recognition/robot.png", im, confidence=0.8)
    img_bwo = None
    goAhead = False
    if (res != None):
...
def slideAndDrop(m,number):
    im = captureMss(m)
    captchaPos = locate("images/captcha/recognition/captchaBar.png", im, confidence=0.7)
    if (captchaPos != None):
        robotPos = locate("images/captcha/recognition/robot.png", im, confidence=0.8)
        newArea = {
        ...
        if(robotPos != None):
            ...
            for x in range(captchaX, robotX, 7):
                ...
                currentNumber = numberOcr(im, "images/captcha/ocr")
...
def start():
    ...
    if (len(gameList) > 0):
        ...
        im = captureMss(m)
        robot = locate("images/captcha/recognition/robot.png", im, confidence=0.8)
        if (robot != None):
            img = sweepScreen(m)
            number = numberOcr(img, "images/captcha/hsvOcr")
